...
TEST_F(metacall_node_class_test, DefaultConstructor)
{
...
#if defined(OPTION_BUILD_LOADERS_NODE)
	{
		...
		/* Static get, set and invoke */
		void *myclass = metacall_class("Test");

		void *class_value = metacall("return_class_function");
		ASSERT_EQ((enum metacall_value_id)METACALL_CLASS, (enum metacall_value_id)metacall_value_id(class_value));

		static const char john[] = "John Doe";

		const void *inner_constructor_params[] = {
		...
		void *constructor_params[] = {
			...
			class_value,
			metacall_value_create_array(inner_constructor_params, sizeof(inner_constructor_params) / sizeof(inner_constructor_params[0]))
		};
		...
		/* Non static get, set and invoke from class object */
