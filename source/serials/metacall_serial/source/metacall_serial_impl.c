...
/*
static void metacall_serial_impl_serialize_value(value v, rapidjson::Value * json_v, rapidjson::Document::AllocatorType & allocator);

static char * metacall_serial_impl_document_stringify(rapidjson::Document * document, size_t * size);

static value metacall_serial_impl_deserialize_value(const rapidjson::Value * v);
*/
...
void metacall_serial_impl_serialize_value(value v, rapidjson::Value * json_v, rapidjson::Document::AllocatorType & allocator)
{
	...
	else if (id == TYPE_ARRAY)
	{
		...
		for (size_t iterator = 0; iterator < array_size; ++iterator)
		{
			...
			metacall_serial_impl_serialize_value(current_value, &json_inner_value, allocator);
	...
	else if (id == TYPE_MAP)
	{
		...
		for (size_t iterator = 0; iterator < map_size; ++iterator)
		{
			...
			metacall_serial_impl_serialize_value(tupla_array[0], &json_member, allocator);

			metacall_serial_impl_serialize_value(tupla_array[1], &json_inner_value, allocator);
...
char * metacall_serial_impl_document_stringify(rapidjson::Document * document, size_t * size)
{
	char * buffer_str;
...
char * metacall_serial_impl_serialize(serial_impl_handle handle, value v, size_t * size)
{
	...
	metacall_serial_impl_serialize_value(v, document, allocator);

	return metacall_serial_impl_document_stringify(document, size);
}
...
value metacall_serial_impl_deserialize_value(const rapidjson::Value * v)
{
	...
	else if (v->IsArray() == true)
	{
		...
		{
			values[index++] = metacall_serial_impl_deserialize_value(it);
		}
	...
	else if (v->IsObject() == true)
	{
		...
		for (rapidjson::Value::ConstMemberIterator it = v->MemberBegin(); it != v->MemberEnd(); ++it)
		{
			const value tupla[] =
			{
				metacall_serial_impl_deserialize_value(&it->name),
				metacall_serial_impl_deserialize_value(&it->value)
			};
