...
loader_handle mock_loader_impl_load_from_file(loader_impl impl, const loader_naming_path path, const loader_naming_name name)
{
	...
	if (handle != NULL)
	{
		log_write("metacall", LOG_LEVEL_DEBUG, "Mock module %s (%s) loaded from file", path, name);
