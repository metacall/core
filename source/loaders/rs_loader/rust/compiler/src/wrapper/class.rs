...
impl Class {
    ...
    pub fn call(&self, attr: &str, args: Vec<MetacallValue>) -> Result<MetacallValue> {
        let attr = self
            .class_methods
            .get(attr)
            .unwrap_or_else(|| panic!("Unable to get {} from {}", attr, self.name));
...
impl Instance {
    ...
    pub fn get_attr(&self, name: &str, class: &Class) -> Result<MetacallValue> {
        let attr = class
            .attributes
            .get(name)
            .unwrap_or_else(|| panic!("Unable to find attribute {} from {}", name, self.name()))
            .clone();
        attr.invoke(self)
    }
    ...
    pub fn call(
        ...
        let method = class
            .get_method(name)
            .unwrap_or_else(|| panic!("Unable to find method {} from {}", name, self.name()));
...
impl ToMetaResult for i64 {
    fn to_meta_result(self) -> Result<MetacallValue> {
        if self < c_long::MIN as i64 || self > c_long::MAX as i64 { return Err(-1);}
    ...
    ($t:ty, $val:expr) => {
        unsafe {
            ...
            match id.try_into() {
                ...
                Err(_) => {
                    eprintln!(
                        "Rust Loader: Return type with id #{} is not implemented, ",
                        id
                    );
