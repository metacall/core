...
{
	void *handle_llvm_data;

} * loader_impl_llvm_handle;

typedef struct loader_impl_llvm_function_type
{
	loader_impl_llvm_handle llvm_handle;
	void *function_llvm_data;
...
{
	loader_impl_llvm_function llvm_function = (loader_impl_llvm_function)impl;
	...
	log_write("metacall", LOG_LEVEL_DEBUG, "Invoking llvm function %s", name);
	...
	if (size > 0)
	{
		...
		log_write("metacall", LOG_LEVEL_DEBUG, "Calling llvm function with arguments (%lu)", size);
	}
	else
	{
		log_write("metacall", LOG_LEVEL_DEBUG, "Calling llvm function without arguments");
	}
...
{
	static struct function_interface_type llvm_interface = {
	...
	return &llvm_interface;
}

int llvm_loader_impl_initialize_types(loader_impl impl)
{
...
{
	loader_impl_llvm llvm_impl;

	(void)impl;
	(void)config;

	llvm_impl = new (struct loader_impl_llvm_type);
	...
	if (llvm_loader_impl_initialize_types(impl) != 0)
	{
		free(llvm_impl);
...
{
	loader_impl_llvm_handle handle = new (struct loader_impl_llvm_handle_type);
...
{
	loader_impl_llvm_handle llvm_handle = (loader_impl_llvm_handle)handle;
...
loader_impl_llvm_function llvm_function_create(loader_impl_llvm_handle llvm_handle)
{
	loader_impl_llvm_function llvm_function = new (struct loader_impl_llvm_function_type);

	if (llvm_function != NULL)
	{
		llvm_function->llvm_handle = llvm_handle;

		llvm_function->function_llvm_data = NULL;
...
{
	loader_impl_llvm llvm_impl = loader_impl_get(impl);

	loader_impl_llvm_handle llvm_handle = (loader_impl_llvm_handle)handle;

	loader_impl_llvm_function llvm_function = llvm_function_create(llvm_handle);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("my_empty_func", 0, llvm_function, &function_llvm_singleton);
	...
	llvm_function = llvm_function_create(llvm_handle);

	if (llvm_function != NULL)
	{
		function f = function_create("two_doubles", 2, llvm_function, &function_llvm_singleton);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("mixed_args", 5, llvm_function, &function_llvm_singleton);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("new_args", 1, llvm_function, &function_llvm_singleton);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("two_str", 2, llvm_function, &function_llvm_singleton);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("three_str", 3, llvm_function, &function_llvm_singleton);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("my_empty_func_str", 0, llvm_function, &function_llvm_singleton);
	...
	if (llvm_function != NULL)
	{
		function f = function_create("my_empty_func_int", 0, llvm_function, &function_llvm_singleton);
