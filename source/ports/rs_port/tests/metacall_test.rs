...
mod tests {
    use metacall::{
        load, MetaCallClass, MetaCallException, MetaCallFunction, MetaCallFuture,
    ...
    fn metacall() {
        ...
        if let Ok(_) = load::from_single_file("py", py_test_file) {
            test_buffer();
            test_class();
            test_object();
            test_pointer();
        }
        if let Ok(_) = load::from_single_file("c", c_test_file) {
            test_char();
            test_double();
            test_float();
            test_int();
        ...
        if let Ok(_) = load::from_single_file("node", js_test_file) {
            test_array();
            test_bool();
            test_exception();
            test_function();
