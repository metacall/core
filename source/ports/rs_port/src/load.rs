...
pub fn from_file(
    ...
    let c_tag = cstring_enum!(tag, LoaderError)?;
    ...
    for script in scripts.into_iter() {
        ...
        if !script_as_pathbuf.exists() {
            return Err(LoaderError::FileNotFound(script_as_pathbuf));
        }
        if !script_as_pathbuf.is_file() {
            return Err(LoaderError::NotAFileOrPermissionDenied(
                script_as_pathbuf,
            ));
        }

        c_script = cstring_enum!(script_as_str, LoaderError)?;
    ...
    {
        return Err(LoaderError::FromFileFailure);
    }
...
/// ```
pub fn from_memory(tag: impl ToString, script: impl ToString) -> Result<(), LoaderError> {
    let script = script.to_string();
    let c_tag = cstring_enum!(tag, LoaderError)?;
    let c_script = cstring_enum!(script, LoaderError)?;
    ...
    {
        return Err(LoaderError::FromMemoryFailure);
    }
