...
- **Test**: [`metacall-python-test`](https://github.com/metacall/core/blob/develop/source/tests/metacall_python_test)
- **Minimum version**: Python >= 3.5, enforced through a compile-time check in [`py_loader_impl.c`](https://github.com/metacall/core/blob/develop/source/loaders/py_loader/source/py_loader_impl.c#L2124). 
##### Runtime behavior
...
Async/await is implemented using Python's standard `asyncio` facilities. The asyncio event loop runs in a separate thread to avoid blocking the calling thread (see [`py_loader_impl.c`](https://github.com/metacall/core/blob/develop/source/loaders/py_loader/source/py_loader_impl.c#L2217)). Results are handled through futures and callbacks.
##### Limitations
