...
# Filter preprocessor flags
string(REGEX REPLACE "( )+" ";" LLVM_CPP_FLAGS_LIST ${LLVM_CPP_FLAGS})

set(LLVM_CPP_FLAGS_FILTER "")

foreach(FLAG ${LLVM_CPP_FLAGS_LIST})
	string(REGEX MATCH "^(-I)((/[^/]+)+)" FLAG_MATCH ${FLAG})

	if("${FLAG_MATCH}" STREQUAL "")
		string(REGEX REPLACE "^(-D)" "" FLAG_FILTER ${FLAG})

		set(FLAG_FILTER "${FLAG_FILTER}=1")

		if(NOT ("${FLAG_FILTER}" STREQUAL ""))
			set(LLVM_CPP_FLAGS_FILTER "${LLVM_CPP_FLAGS_FILTER} ${FLAG_FILTER}")
		endif()
	endif()
endforeach()

set(LLVM_CPP_FLAGS ${LLVM_CPP_FLAGS_FILTER})

if(NOT LLVM_FIND_QUIETLY)
	message(STATUS "Found LLVM version: ${LLVM_VERSION}")
	message(STATUS "LLVM preprocessor flags: ${LLVM_CPP_FLAGS}")
endif(NOT LLVM_FIND_QUIETLY)

# Handle the QUIETLY and REQUIRED arguments and set LLVM_FOUND to TRUE if
# all listed variables are TRUE
find_package_handle_standard_args(LLVM DEFAULT_MSG LLVM_LIBRARIES LLVM_INCLUDE_DIRS LLVM_LIBRARY_DIRS)
