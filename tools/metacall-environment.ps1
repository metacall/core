...
# Install clang-format 12 on Windows.
# Strategy:
#   1. Download the standalone clang-format.exe from the LLVM 12.0.1 GitHub release.
#   2. Place it in a dedicated directory and add that directory to PATH.
#   3. Also copy it as clang-format-12.exe so cmake/FindClangFormat.cmake finds it
#      by its versioned name.
#   4. If Docker Desktop is available and the download fails, install a wrapper
#      batch file that proxies calls through ghcr.io/jidicula/clang-format:12.
function Set-ClangFormat {
	Write-Output "Installing clang-format 12"

	Set-Location $ROOT_DIR

	$ClangFormatVersion = "12.0.1"
	$ClangFormatDir     = "$env:ProgramFiles\clang-format-12"
	$DepsDir            = "$ROOT_DIR\dependencies"

	# LLVM publishes standalone clang-format binaries on GitHub Releases.
	$DownloadUrl = "https://github.com/llvm/llvm-project/releases/download/llvmorg-$ClangFormatVersion/clang-format-$ClangFormatVersion.exe"

	mkdir -Force $DepsDir    | Out-Null
	mkdir -Force $ClangFormatDir | Out-Null

	$ExeDest = "$DepsDir\clang-format-$ClangFormatVersion.exe"

	$NativeInstalled = $false

	# --- Native binary ---
	if (!(Test-Path -Path $ExeDest)) {
		Write-Output "Downloading clang-format $ClangFormatVersion from LLVM GitHub releases..."
		try {
			(New-Object Net.WebClient).DownloadFile($DownloadUrl, $ExeDest)
		} catch {
	...
	if (Test-Path -Path $ExeDest) {
		# Expose as both the versioned and the unversioned name.
		Copy-Item -Force $ExeDest "$ClangFormatDir\clang-format-12.exe"
		Copy-Item -Force $ExeDest "$ClangFormatDir\clang-format.exe"

		Add-to-Path $ClangFormatDir

		Write-Output "clang-format 12 installed at $ClangFormatDir"
		$NativeInstalled = $true
	} else {
		Write-Output "Native clang-format binary not available."
	}
	...
	if (-not $NativeInstalled) {
		...
		if ($null -ne $DockerExe) {
			Write-Output "Docker Desktop detected. Installing Docker wrapper for clang-format 12..."

			$DockerImage = "ghcr.io/jidicula/clang-format:12"

			# Pull the image now so the first invocation is not slow.
			docker pull $DockerImage

			# a .cmd wrapper that forwards all arguments into the container.
			$WrapperContent = @"
@echo off
REM Auto-generated by metacall-environment.ps1 -- DO NOT EDIT.
REM Proxies clang-format calls through the pinned Docker image.
docker run --rm ^
    -v "%CD%:%CD%" ^
    -w "%CD%" ^
    "$DockerImage" ^
...
"@
			...
			$WrapperDir = "$env:ProgramFiles\clang-format-12"
			mkdir -Force $WrapperDir | Out-Null

			$WrapperContent | Out-File -Encoding ascii "$WrapperDir\clang-format-12.cmd"
			$WrapperContent | Out-File -Encoding ascii "$WrapperDir\clang-format.cmd"

			Add-to-Path $WrapperDir

			Write-Output "Docker wrapper installed at $WrapperDir"
		} else {
			Write-Output "WARNING: clang-format 12 could not be installed."
			Write-Output "Please install it manually:"
			Write-Output "  Option A (native):  Download from https://github.com/llvm/llvm-project/releases/tag/llvmorg-12.0.1"
			Write-Output "  Option B (Docker):  Install Docker Desktop, then re-run this script."
			Write-Output "  Option C (winget):  winget install LLVM.LLVM --version 12.0.1"
			Write-Output "  Option D (choco):   choco install llvm --version 12.0.1"
		}
	}
}
