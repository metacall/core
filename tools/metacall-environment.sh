...
sub_clangformat(){
	...
	CLANG_FORMAT_INSTALLED=0

	# Helper: install a Docker-based wrapper script at /usr/local/bin/clang-format-12
	# so that cmake/FindClangFormat.cmake and the git hook find a callable binary.
	install_docker_wrapper(){
		if ! command -v docker >/dev/null 2>&1; then
			echo "clang-format-12 could not be installed natively and Docker is not available."
			echo "Please install Docker or clang-format-12 manually."
			return 1
		fi

		echo "Installing Docker-based clang-format-${CLANG_FORMAT_VERSION} wrapper..."

		# Pull the image eagerly so first use is not slow.
		docker pull "${CLANG_FORMAT_DOCKER_IMAGE}" || true

		# a thin wrapper that forwards all arguments into the container.
		$SUDO_CMD tee /usr/local/bin/clang-format-${CLANG_FORMAT_VERSION} > /dev/null << 'DOCKER_WRAPPER'
#!/bin/sh
# Auto-generated by metacall-environment.sh - DO NOT EDIT.
...
DOCKER_WRAPPER
		...
		$SUDO_CMD chmod +x /usr/local/bin/clang-format-${CLANG_FORMAT_VERSION}

		# Provide an unversioned alias only when there is no system clang-format
		# so we do not shadow a potentially newer binary used for other purposes.
		if ! command -v clang-format >/dev/null 2>&1; then
			$SUDO_CMD ln -sf /usr/local/bin/clang-format-${CLANG_FORMAT_VERSION} \
				/usr/local/bin/clang-format
		fi

		echo "Docker wrapper installed at /usr/local/bin/clang-format-${CLANG_FORMAT_VERSION}"
		CLANG_FORMAT_INSTALLED=1
	}
	...
	if [ "${OPERATIVE_SYSTEM}" = "Linux" ]; then
		if [ "${LINUX_DISTRO}" = "debian" ] || [ "${LINUX_DISTRO}" = "ubuntu" ]; then
			...
			# Try the LLVM apt repository first (guaranteed to have clang-format-12).
			if wget -qO- https://apt.llvm.org/llvm-snapshot.gpg.key \
					| $SUDO_CMD tee /etc/apt/trusted.gpg.d/apt.llvm.org.asc > /dev/null 2>&1; then
				$SUDO_CMD sh -c "echo \"deb http://apt.llvm.org/${CODENAME}/ llvm-toolchain${LINKNAME}-${CLANG_FORMAT_VERSION} main\" >> /etc/apt/sources.list"
				$SUDO_CMD sh -c "echo \"deb-src http://apt.llvm.org/${CODENAME}/ llvm-toolchain${LINKNAME}-${CLANG_FORMAT_VERSION} main\" >> /etc/apt/sources.list"
				$SUDO_CMD apt-get update
				if $SUDO_CMD apt-get install -y --no-install-recommends clang-format-${CLANG_FORMAT_VERSION}; then
					# Provide an unversioned alias only when absent.
					if [ ! -e /usr/bin/clang-format ]; then
						$SUDO_CMD ln -s /usr/bin/clang-format-${CLANG_FORMAT_VERSION} /usr/bin/clang-format
					fi
					CLANG_FORMAT_INSTALLED=1
				fi
			fi

			# Fall back to Docker wrapper when the apt install failed
			# (e.g. network error, unsupported codename).
			if [ $CLANG_FORMAT_INSTALLED -eq 0 ]; then
				echo "Native apt install of clang-format-${CLANG_FORMAT_VERSION} failed, trying Docker wrapper..."
				install_docker_wrapper
			fi

		elif [ "${LINUX_DISTRO}" = "alpine" ]; then
			# Alpine 3.15 ships clang-extra-tools 12.0.1;
			# later versions.  Try the pinned version first, fall back to Docker.
			if $SUDO_CMD apk add --no-cache \
					--repository=https://dl-cdn.alpinelinux.org/alpine/v3.15/main \
					clang-extra-tools=12.0.1-r1 2>/dev/null; then
				CLANG_FORMAT_INSTALLED=1
			else
				echo "Alpine apk install of clang-extra-tools=12.0.1-r1 failed, trying Docker wrapper..."
				install_docker_wrapper
			fi

		elif [ "${LINUX_DISTRO}" = "fedora" ]; then
			# Fedora ships clang-tools-extra which includes clang-format.
			# The exact version depends on the Fedora release; verify after install.
			if $SUDO_CMD dnf install -y clang-tools-extra 2>/dev/null; then
				installed_major=$(clang-format --version 2>/dev/null | grep -oE '[0-9]+' | head -1)
				if [ "${installed_major}" = "${CLANG_FORMAT_VERSION}" ]; then
					CLANG_FORMAT_INSTALLED=1
				else
					echo "Fedora provided clang-format ${installed_major}, but ${CLANG_FORMAT_VERSION} is required."
					install_docker_wrapper
				fi
			else
				install_docker_wrapper
			...
			# RHEL / CentOS Stream - try clang-tools-extra from CodeReady/AppStream.
			if $SUDO_CMD yum install -y clang-tools-extra 2>/dev/null || \
			   $SUDO_CMD dnf install -y clang-tools-extra 2>/dev/null; then
				installed_major=$(clang-format --version 2>/dev/null | grep -oE '[0-9]+' | head -1)
				if [ "${installed_major}" = "${CLANG_FORMAT_VERSION}" ]; then
					CLANG_FORMAT_INSTALLED=1
				else
					echo "System provided clang-format ${installed_major}, but ${CLANG_FORMAT_VERSION} is required."
					install_docker_wrapper
				fi
			else
				install_docker_wrapper
		...
		elif [ "${LINUX_DISTRO}" = "opensuse" ] || [ "${LINUX_DISTRO}" = "suse" ]; then
			if $SUDO_CMD zypper install -y clang12 2>/dev/null; then
				CLANG_FORMAT_INSTALLED=1
			else
				install_docker_wrapper
			fi
			...
			# Unknown Linux distribution - go straight to Docker wrapper.
			echo "Unrecognised Linux distribution '${LINUX_DISTRO}'. Trying Docker wrapper..."
			install_docker_wrapper
		fi

	elif [ "${OPERATIVE_SYSTEM}" = "Darwin" ]; then
		# macOS - Homebrew provides llvm@12 as a versioned formula.
		if command -v brew >/dev/null 2>&1; then
			brew install llvm@12

			LLVM12_PREFIX="$(brew --prefix llvm@12)"

			# Expose the versioned binary name that cmake/FindClangFormat.cmake
			# and the git hook both search for.
			if [ ! -e /usr/local/bin/clang-format-${CLANG_FORMAT_VERSION} ]; then
				$SUDO_CMD ln -sf "${LLVM12_PREFIX}/bin/clang-format" \
					/usr/local/bin/clang-format-${CLANG_FORMAT_VERSION}
			fi

			# Add llvm@12 bin dir to the current shell session so subsequent
			# steps in the same script invocation pick it up immediately.
			export PATH="${LLVM12_PREFIX}/bin:${PATH}"

			CLANG_FORMAT_INSTALLED=1
		else
			echo "Homebrew not found on macOS. Trying Docker wrapper..."
			install_docker_wrapper
		fi
	fi

	if [ $CLANG_FORMAT_INSTALLED -eq 0 ]; then
		echo "Warning: clang-format-${CLANG_FORMAT_VERSION} could not be installed." \
			"Formatting will be unavailable."
	fi
}
