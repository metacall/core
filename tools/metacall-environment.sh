...
sub_python(){
	...
	if [ "${OPERATIVE_SYSTEM}" == "Linux" ]; then
		...
		source ~/.bash_profile
		mkdir -p build
		ENV_PATH=$ROOT_DIR/build/CMakeConfig.txt
		echo "-DPython3_INCLUDE_DIRS=""$HOME/.pyenv/versions/3.11.1/include/python3.11""" >> $ENV_PATH
		echo "-DPython3_LIBRARY=""$HOME/.pyenv/versions/3.11.1/lib/libpython3.11.dylib""" >> $ENV_PATH
		echo "-DPython3_EXECUTABLE=""$HOME/.pyenv/versions/3.11.1/bin/python3.11""" >> $ENV_PATH
		echo "-DPython3_ROOT=""$HOME/.pyenv/versions/3.11.1""" >> $ENV_PATH
		echo "-DPython3_VERSION=""3.11.1""" >> $ENV_PATH
		echo "-DPython3_FIND_FRAMEWORK=""NEVER""" >> $ENV_PATH
...
sub_ruby(){
	...
	if [ "${OPERATIVE_SYSTEM}" == "Linux" ]; then
		...
		echo "-DRuby_INCLUDE_DIR=""$RUBY_PREFIX/include/ruby-3.2.0""" >> $ENV_PATH
		echo "-DRuby_LIBRARY=""$RUBY_PREFIX/lib/libruby.3.2.dylib""" >> $ENV_PATH
		echo "-DRuby_EXECUTABLE=""$RUBY_PREFIX/bin/ruby""" >> $ENV_PATH
		echo "-DRuby_VERSION_STRING=""$RUBY_VERSION""" >> $ENV_PATH
	fi
}
